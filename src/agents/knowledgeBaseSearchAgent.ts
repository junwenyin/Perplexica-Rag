...
const basicKnowledgeBaseSearchResponsePrompt = `
...
const createBasicKnowledgeBaseSearchAnsweringChain = (
  llm: BaseChatModel,
  ...
  return RunnableSequence.from([
    RunnableMap.from({
      ...
      context: RunnableSequence.from([
        ...
        basicKnowledgeBaseSearchRetrieverChain
          .pipe(rerankDocs)
          .withConfig({
    ...
    ChatPromptTemplate.fromMessages([
      ['system', basicKnowledgeBaseSearchResponsePrompt],
...
const basicKnowledgeBaseSearch = (
  query: string,
  history: BaseMessage[],
  llm: BaseChatModel,
  embeddings: Embeddings,
  ...
  try {
    const basicKnowledgeBaseSearchAnsweringChain =
      createBasicKnowledgeBaseSearchAnsweringChain(llm, embeddings);

    const stream = basicKnowledgeBaseSearchAnsweringChain.streamEvents(
      {
...
const handleKnowledgeBaseSearch = (
  message: string,
  history: BaseMessage[],
  llm: BaseChatModel,
  embeddings: Embeddings,
) => {
  const emitter = basicKnowledgeBaseSearch(message, history, llm, embeddings);
  return emitter;
};

export default handleKnowledgeBaseSearch;
